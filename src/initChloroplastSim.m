...
switch mode
    ...
    case 'PSIITrapLake'
        if( params.includeOSM ~= 1 )
            vars={   'PSII' 'qE' 'PQ'   'cytb6f' 'PSI' 'MV'  'ATPsynthase' 'LumenFlux' 'KeaFlux' };
        else
            vars={   'PSII' 'qE' 'PQ'   'cytb6f' 'PSI' 'MV'  'ATPsynthase' 'LumenFlux' 'Volume' 'KeaFlux' };
        end
...
NADPHstart=0.1;
ATPstart= 2;
OSMStart = params.LumenMgStart + params.LumenClStart + params.LumenKStart;
%%
%% Describe Modules
...
if( params.includeOSM ~=1 )
    ...
    sp.abstol.ATPsynthase =[                          1e-6     1e-5 1e-6];
    sp.function.ATPsynthase=@evolveATPsynthase;
    sp.inputs.ATPsynthase ={  'LumenMg'         'LumenCl'       'LumenK' 'Fdxr'   'Volume'     };
end
...
if( params.includeOSM ~= 1 )
    ...
    sp.abstol.LumenFlux =[   1e-5                 1e-5             1e-5   ];
    sp.function.LumenFlux=@evolveLumen;
    sp.inputs.LumenFlux ={    'LumenProtons' 'Volume' };
end
%% Kea
sp.varnames.KeaFlux = { 'LumenProtons','LumenK','Volume' };
sp.init.KeaFlux = [ lumenProtonsStart,params.LumenKStart,params.LumenVolumeStart ];
sp.abstol.KeaFlux = [1e-6,1e-5,1e-6];
sp.function.KeaFlux = @evolveKea;
sp.inputs.KeaFlux = {'LumenMg','LumenCl'};

%% Volume
if( params.includeOSM == 1 )
sp.varnames.Volume = {'LumenK','LumenCl','LumenMg','LumenProtons','Volume'};
sp.init.Volume = [ params.LumenKStart,params.LumenClStart,params.LumenMgStart,lumenProtonsStart params.LumenVolumeStart];
sp.abstol.Volume = [ 1e-5 1e-5 1e-5 1e-6 1e-6];
sp.function.Volume = @evolveVolume;
sp.inputs.Volume = {};
end
