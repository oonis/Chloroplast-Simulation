...
%% Check for directories and param. Create directories if missing

defaultOutDir     = './simulations/';
defaultParamsFile = './params.txt';
defaultOutFigFolder = './figures/';

if( exist( defaultOutDir,'dir' ) == 7 )
    disp( 'simulations found' );
else
    disp('creating ./simulations/');
    mkdir( './','simulations' );
end

if( exist( defaultOutFigFolder,'dir' ) == 7 )
    disp( 'figures found' );
else
...
if( exist( defaultParamsFile,'file' ) == 0 )
    disp('ERROR: params file not found');
    return;
end
...
paramValueDisp = uicontrol('Parent', paramlisth, 'Style', 'edit', 'String', params.(paramfields{1}), ...
    'Position', [xPos, yPos(5), w*0.5, Lh], 'fontsize', cfontsize, 'BackgroundColor', [1 1 1],'Callback',@alterValue);
...
%     'Callback', {@setParamToVary}, 'Value', 0, 'fontsize', cfontsize);
    ...
    function alterValue( bh,userData )
        allParameters = get( parameter,'String' );
        currSelectedParameter = allParameters{ get(parameter,'Value') };
        params.(currSelectedParameter) = str2double( get(paramValueDisp,'String') );
        
        disp( horzcat( currSelectedParameter,' is now ',num2str(get(paramValueDisp,'String')) ) );
...
loadPamButton = uicontrol('Parent', lightPAM, 'Style', 'pushbutton', ...
    ...
    function loadPamIntensities(~, ~, acth, sath, flashlengthh)
...
buttonPositionSize   =  [ 190 5 180 50];
...
runSimButton    = uicontrol( 'Parent', runSimPanel,'Style', 'pushbutton', ...
    ...
    'Callback', {@runSimulation,ah}, 'fontsize', 12);

runMultiSimButton = uicontrol( 'Parent', runSimPanel,'Style', 'pushbutton', ...
    'String', 'Run Batch Simulations', 'Position', [8,5,180,50], ...
    'Callback', {@runMultiSimulation,ah}, 'fontsize', 12);
...
paramName='';
    function plotSimulationVar(bh, ~, ah)
        ...
        hold(ah, ud.hold); % Will all of these sizes always be one?
        for k=1:size(npqsim,1)
            for kk=1:size(npqsim,2)
                for kkk=1:size(npqsim,3)
...
%% Run batch simulation
    function runMultiSimulation( bh,userData,ah )
        [newName,newPath] = uigetfile( '*.txt','Select simulation file' );
        newParameterFilePath = fullfile(newPath,newName);
        
        fid = fopen( newParameterFilePath );
        
        scanstring='%s%s%s';
        [paramstrings, count]=textscan(fid, scanstring,'CommentStyle', '%');
        labels = paramstrings{1};
        values = paramstrings{2};
        x = 1;
        while( x <= numel(paramstrings{1}) )
            params = initParams;
            temp = labels{x};
            if( strcmpi(temp,'simname') == 1 ) % Start the reading!
                disp( horzcat('Reading in simulation ',values{x}) );
                simPath = fullfile( './','simulations',horzcat(values{x},'.txt') ); % The simulations file should already exist. No need to check again.
                x = x + 1;
                while( x <= numel(paramstrings{1}) )
                    paramName = labels{x};
                    
                    if( strcmpi(paramName,'simname') == 1 )
                        x = x - 1;
                        break;
                    end
                    
                    try
                        params.(paramName) = str2num( values{x} );
                        disp( horzcat('Changing ',paramName,' to ',values{x}) );
                    catch
                        disp( horzcat('unable to find parameter:: ',labels{x}) );
                    end
                    x = x + 1;
                    
                end
                runSimulation( bh,userData,ah,simPath );
            end
            x = x + 1;
        end
                
%         disp('Batch simulation completed');
%         disp( ['<font color="green">','Batch Simulation completed!','</font>'] );
        cprintf( '*blue','Batch simulation complete!\n' );
        
    end


%% Run Simulation Functions
   function runSimulation(bh, userData, ah,simPath)
       if( nargin < 4 ) % We just want a single run
           simPath = '';
       end
        ...
        for k = 1:length(ud.quenchModels)
            for kk = 1:size(ud.intensity,1)
                for kkk = 1: nVaryParams
                    ...
                    % Post processing
%                     tempSim = postProcesses(npqsim{k,kk,kkk});
%                     npqsim{k,kk,kkk} = tempSim;
                    
%                     plotFluorescenceYield(npqsim{k,kk,kkk}, ah, [colors(kkk) symbols(k)]); % NOTE: this is why it always displays that one plot
                    hold(ah, 'on')
                    toc
        ...
        % store all values (if needed)
        if( strcmp(simPath,'') ~= 1 )
            disp( horzcat('Saving all data to ',simPath) );
            getAllValues( npqsim{1},simPath );
        end
